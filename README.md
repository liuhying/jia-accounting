...
| 💰 **应收应付** | 账龄分析 (6桶/1-30/31-60/61-90/91-180/181-365/365+) |
| 🏦 **银行对账** | CSV导入 · 自动匹配 · 手工核销 |
| 📅 **期末结转** | 自动损益结转 · 反结转 · 结账状态 |
...
│          Nginx (reverse proxy)           │
├─────────────────────────────────────────┤
│  accounting_index.html (Vue 2.7 SPA)    │  ← 单文件 14个Tab零跳转
│  Element UI + Axios                      │
├─────────────────────────────────────────┤
...
- **v-show 大规模模板**: 14个面板全部 v-show，避免 Vue 2.7 大模板 v-if 编译异常
